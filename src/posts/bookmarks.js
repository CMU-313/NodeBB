...
// Helper function at module level
async function hasBookmarked(pid, uid) {
	if (parseInt(uid, 10) <= 0) {
...
module.exports = function (Posts) {
	Posts.bookmark = async function (pid, uid) {
		return await toggleBookmark('bookmark', pid, uid, Posts);
	};

	Posts.unbookmark = async function (pid, uid) {
		return await toggleBookmark('unbookmark', pid, uid, Posts);
	};

	Posts.hasBookmarked = async function (pid, uid) {
		return await hasBookmarked(pid, uid);
	};
};

...
async function deleteUploads(callerUid, uid, User) {
...
function createUserDeleteContext(User, deletesInProgress) {
	return { User, deletesInProgress };
}

async function userDeleteContent(callerUid, uid, context) {
	const { deletesInProgress, User } = context;
	...
	await deleteUploads(callerUid, uid, User);
	await deleteQueued(uid);
	delete deletesInProgress[uid];
}

async function userDeleteAccount(uid, context) {
	const { deletesInProgress, User } = context;
...
async function userDelete(callerUid, uid, context) {
	const { User } = context;
	await User.deleteContent(callerUid, uid);
	return await User.deleteAccount(uid);
}

function setupUserDelete(User, deletesInProgress) {
	const context = createUserDeleteContext(User, deletesInProgress);
	User.delete = async (callerUid, uid) => userDelete(callerUid, uid, context);
	User.deleteContent = async (callerUid, uid) => userDeleteContent(callerUid, uid, context);
	User.deleteAccount = async uid => userDeleteAccount(uid, context);
}

module.exports = function (User) {
	const deletesInProgress = {};
	setupUserDelete(User, deletesInProgress);
};